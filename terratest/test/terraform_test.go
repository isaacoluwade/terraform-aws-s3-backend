...
// TestS3BackendDefaultApply is the canonical happy-path Terratest. It applies
// the module with minimum-required vars, asserts the deliverable resources
// exist via direct AWS API calls, then destroys.
func TestS3BackendDefaultApply(t *testing.T) {
	t.Parallel()
	...
	assert.NotEmpty(t, lockTableName, "lock_table_name output should be populated")
...
// TestS3BackendDREnabled exercises the cross-region replication path. It writes
// a test object to the primary bucket and waits for it to appear on the DR
// side, confirming replication actually replicates.
func TestS3BackendDREnabled(t *testing.T) {
	t.Parallel()
